...
public class SystemTalk : GenericFunction {
	...
	public bool mouseIsReady = false;
	
	public void Start(){
		gameObject.GetComponent<SystemTalk>().enabled = false;
	}
	...
	public void startFromHere(){
        ...
        validateNpcInfo(gameObject.name , h);
	...
	public void calculeAll(int m , int h){
	...
	public void validateNpcInfo(string name , int h){
	...
	public void validateLifeTime(string time, string fileName, bool newValue){
		...
		if(!gameObject.GetComponent<DemoAI>()){
			...
			gameObject.GetComponent<DemoAI>().StartTalk();
		}else{
